/*
 * Copyright (c) 2022 chr_56 & Abou Zeid (kabouzeid),Andrew Neal (original authors)
 */
 ...
 */
class AudioPlayer(context: Context) : Playback, MediaPlayer.OnErrorListener, MediaPlayer.OnCompletionListener {
    ...
    private var currentMediaPlayer = MediaPlayer().also { it.setWakeMode(context, PowerManager.PARTIAL_WAKE_LOCK) }
    ...
    override fun setDataSource(path: String): Boolean {
        isReady = false
        isReady = setDataSourceImpl(currentMediaPlayer, path)
        if (isReady) {
     ...
     */
    override fun isPlaying(): Boolean = isReady && currentMediaPlayer.isPlaying
     ...
     */
    override fun processTimeAxis(): Int =
        if (!isReady) { -1 } else
            try {
