...
object Songs : ISongs, Delegated<ISongs>() {
    ...
    override fun onCreateDelegate(context: Context): ISongs {
        ...
        val impl: ISongs = if (backend.useMediaStoreSongs) MediaStoreSongs else RoomSongs
        return impl
    }
    ...
    override suspend fun lastest(context: Context): Song? = delegate(context).lastest(context)

    override suspend fun total(context: Context): Int = delegate(context).total(context)
}