...
abstract class QueryDao {
    ...
    fun genresWithName(genresName: String, sortMode: SortMode): List<GenreEntity> = searchGenreEntity(
        SimpleSQLiteQuery(
            "SELECT * from $GENRES where $GENRE like ? order by ${roomGenreQuerySortOrder(sortMode)}",
            arrayOf<Any>("%$genresName%")
        )
    )
    ...
    @RawQuery
    protected abstract fun searchGenreEntity(query: SupportSQLiteQuery): List<GenreEntity>
