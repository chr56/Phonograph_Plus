...
**太长不看**: 若设备 Android 版本大于 9，可安装带有 `IntermediaRelease` 的安装包实现平滑过渡；否则，必须先卸载旧版才能升级到 v2.0。

由于各种原因([^skr])，自 v2.0.0 开始，决定**更换**签名密钥：

|       | 序列号                  | SHA-256 指纹                                                       |
|-------|----------------------|------------------------------------------------------------------|
| 旧签名密钥 | 0x3BEE71AB           | 01654EB9348B2C802FCBF21282DE0CDCA2A226337B9A2FA201E3AC376B0CE76C |
| 新签名密钥 | 0x00FFE5FCD512F9C835 | 5DACA926C8E9BB628FFEF98B7A228C6564CCC29ED5031695A030E15120271AB7 |

因此，一般地，需要**先卸载**才能升级到 v2.0；

然而，对于使用 **Android 9** 及以上版本的用户，现准备了一个版本 **`IntermediateRelease`**：
其使用轮换密钥签名（同时包含旧密钥和新密钥及其 rotation lineage），使得用户可以**直接安装**而无需先卸载实现平滑过渡; 
但此功能仅适用于 Android 9 及以上版本 (_APK signature scheme v3_)，使用旧设备的用户仍然需要先卸载才能安装。

[^skr]: * 旧密钥过于“私人”（同时用于其他个人项目），且其证书已过期（但通常系统不会验证）
